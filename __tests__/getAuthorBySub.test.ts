...
describe('getAuthorBySub', () => {
  ...
  it('matches keycloak://user/{uuid} against stored core://user/sub/{uuid}', () => {
    const result = getAuthorBySub(mockAuthors, 'keycloak://user/cf8eb669-0c0f-432d-8fdf-b479ac2082a1')
    expect(result).toEqual(mockAuthors[2])
  })

  it('matches keycloak://user/{id} against stored core://user/{id}', () => {
    const canonicalAuthors: IDBAuthor[] = [
      {
        id: 'author-c',
        name: 'Canonical Author',
    ...
    expect(
      getAuthorBySub(canonicalAuthors, 'keycloak://user/9999')
    ).toEqual(canonicalAuthors[0])
  })
