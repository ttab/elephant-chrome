...
describe('handlers', () => {
  ...
  describe('ScheduleDecoratorUpdate', () => {
    it('should not execute if uuid missing', async () => {
      ...
      await scheduler.execute(update)
    ...
    it('should not execute if parent is null', async () => {
      ...
      await scheduler.execute(update, null)

      expect(runUpdateDecorators).not.toHaveBeenCalled()
      expect(setData).not.toHaveBeenCalled()
    })

    it('should call runUpdateDecorators and update state', async () => {
      ...
      const setData = vi.fn() as unknown as Dispatch<SetStateAction<DocumentStateWithDecorators[]>>
      ...
      await scheduler.execute(update, parent)
