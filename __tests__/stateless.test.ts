...
describe('stateless', () => {
  ...
  it('should create and parse a stateless context message', () => {
    const statelessMsg = createStateless(StatelessType.CONTEXT, { preview: true, id: '123' })
    expect(statelessMsg).toEqual(`${StatelessType.CONTEXT}@${JSON.stringify({ preview: true, id: '123' })}`)

    const parsed = parseStateless(statelessMsg)
    expect(parsed).toEqual({ type: StatelessType.CONTEXT, message: { preview: true, id: '123' } })
  })
