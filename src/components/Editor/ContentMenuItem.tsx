...
export const ContentMenuItem = ({ action }: { action: TBPluginRegistryAction }): JSX.Element => {
  return (
    <Menu.Item
      action={action.name}
      className='grid gap-x-5 py-[0.4rem] border group grid-cols-[1.5rem_minmax(max-content,220px)_minmax(max-content,90px)] rounded cursor-default border-white hover:border-gray-200 hover:bg-gray-100 dark:border-slate-900 dark:hover:border-slate-700 dark:hover:bg-slate-800'
    >
