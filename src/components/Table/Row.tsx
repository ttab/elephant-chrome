...
export const Row = ({ row, handleOpen, openDocuments, type }: {
  ...
  return (
    <TableRow
      ...
      data-state={getDataState(openDocuments, wire, type, row)}
      onClick={(event: MouseEvent<HTMLTableRowElement>) => handleOpen(event, row)}
      ref={(el) => handleRef(el, row, currentModal, type)}
    >
...
function handleRef(el: HTMLTableRowElement | null, row: RowType<unknown>, currentModal: ModalData | undefined, type: DocumentType): void {
