...
export const usePrintArticles = ({ filter, page, id }: {
  filter?: QueryParams
  page?: number
  id?: string
  ...
  const fetcher = (): Promise<PrintArticle[] | undefined> =>
    fetch({ index, repository, session, filter, page })

  const { data, error } = useSWR<PrintArticle[] | undefined, Error>(['tt/print-articles', key, workflowStatus], fetcher)
