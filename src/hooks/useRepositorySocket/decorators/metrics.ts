...
import type { DocumentMetrics as APIDocumentMetrics } from '@ttab/elephant-api/repository'
...
/**
 * Re-export the API's DocumentMetrics for direct access if needed.
 */
export type { APIDocumentMetrics as DocumentMetrics }
 ...
 * const metricsDecorator = createMetricsDecorator({
 *   kinds: ['char_count', 'word_count'],
 *   repository: repositorySocket.repository
 * })
 *
 * const { data } = useRepositorySocket({
 *   type: 'core/article',
 *   decorators: [metricsDecorator]
 * })
 *
 * // Access metrics data
 * const charCount = data[0].decoratorData?.metrics?.charCount
 * ```
 */
export function createMetricsDecorator(options: {
  ...
  return {
    /**
     * Fetches metrics for all included documents when a document updates.
     *
     * Performs batch API call for all included documents at once.
     * Returns a Map containing metrics for all included documents.
     */
    ...
    /**
     * Fetches metrics for updated document
     *
     * When a document updates, re-fetch metrics for current document
     * This ensures metric data stays current as deliverables are modified.
     */
    async onUpdate(update: DocumentUpdate) {
      ...
      return fetchMetricsForUuids([uuid], repository, kinds)
    }
  }
}
