...
import type { DocumentStateWithDecorators } from './types'
import {
 ...
 * })
 *
 * // Type-safe access to decorator data by UUID
 * const articleUuid = data[0].document?.uuid
 * const metrics = articleUuid ? data[0].decoratorData?.[articleUuid] : undefined
 * const charCount = metrics?.charCount
 * ```
 */
export function useRepositorySocket<TEnrichment = unknown>({
  from,
  to,
  include,
  type,
  ...
  data: DocumentStateWithDecorators<TEnrichment>[]
  error: Error | null
  isLoading: boolean
} {
  const { data: session } = useSession()
  const { repositorySocket } = useRegistry()
  const [data, setData] = useState<DocumentStateWithDecorators<TEnrichment>[]>([])
  const { setData: setTableData } = useTable<DocumentStateWithDecorators>()
    ...
    const fetchDocuments = async () => {
      try {
        ...
        if (decoratorsRef.current.length > 0) {
          void (async () => {
            try {
              const enrichedDocs = await runInitialDecorators<TEnrichment>(
                documents,
      ...
      setData((prevData) => {
        ...
        if (isDocumentUpdate(update)) {
          return handleDocumentUpdate(
            prevData,
            update,
            new ScheduleDecoratorUpdate(setData, decoratorsRef, runUpdateDecorators)
          )
        }
        return prevData
      })
  ...
  // We only want to re-run this effect when accessToken changes
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [repositorySocket, from, to, include, type, asTable])
