...
export const AuthorNames = ({ assignment }: { assignment: AssignmentInterface }): JSX.Element => {
  ...
  const enhancedFull = useMemo(() => {
    ...
    if (statusValue !== 'draft' && statusValue !== 'done') {
      return full
        ? `${full}, ${t('core:labels.' + statusValue)} ${t('shared:authors.from', { author: lastStatusUpdateAuthor.name })}`
        : `${t('core:labels.' + statusValue)} ${t('shared:authors.from', { author: lastStatusUpdateAuthor.name })}`
    }
    return full
