...
const meta: ViewMetadata = {
  name: 'ApprovalsBeta',
  path: `${import.meta.env.BASE_URL}/approvals-beta`,
  widths: {
    sm: 12,
    md: 12,
    lg: 6,
...
export const ApprovalsBeta = (): JSX.Element => {
  ...
  return (
    <View.Root tab={currentTab} onTabChange={setCurrentTab}>
      <ViewHeader.Root>
        <ViewHeader.Content>
          <ViewHeader.Title name='ApprovalsBeta' title={t('views:approvals.title')} />
          <Header type='ApprovalsBeta' />
        </ViewHeader.Content>
        <ViewHeader.Action />
      </ViewHeader.Root>
      ...
      <View.Content variant='grid' columns={slots.length}>
        {slots.map((slot, colN) => (
          <View.Column key={slot.key}>
            ...
            {!isLoading && structuredData[colN]?.items.map((item) => {
              ...
              return (
                <ApprovalsCard
                  key={item.id}
                  item={item}
                  status={getStatusSpecifications(item._deliverable?.status || 'draft')}
