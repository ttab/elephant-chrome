...
export async function createFlash({
  ...
  if (!updatedPlanningId) {
    throw new Error(`createAssignmentError: ${ydoc.id}`)
  }

  // A complementary text assignment (2 på 2) is co-created for a quick first version
  const twoOnTwoData = documentStatus === 'usable' ? { title: flashTitle, text: flashBodyText, deliverableId: crypto.randomUUID() } : undefined
  console.log('🚀 ~ :89 ~ createFlash ~ twoOnTwoData:', twoOnTwoData)
