...
interface CalendarGridColumnProps {
  date: Date
  items: CalendarType[]
}
...
export const CalendarGridColumn = ({ date, items }: CalendarGridColumnProps): JSX.Element => {
    ...
    <div>
      ...
      <div className="border-r h-full flex flex-col p-4 gap-8">
        {items.map(item => {
          ...
          const id = item._id
          const sector = CalendarSectors.find((sector) => sector.value === item._source['document.rel.story.uuid'][0])
