...
interface CalendarTableProps<TData, TValue> {
...
export const CalendarTable = <TData, TValue>({
  onRowSelected
}: CalendarTableProps<TData, TValue>): JSX.Element => {
  const { isActive: isActiveView } = useView()
  const { state, dispatch } = useNavigation()
  const { viewId: origin } = useView()

  const { table, loading } = useCalendarTable()
